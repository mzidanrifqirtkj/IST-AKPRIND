...
typedef data_as zidan[20];//deklarasi larik
//deklarasi variabel
zidan isi;
int n,i,pil;
...
void add_asesor(zidan &X){
	...
	cout<<"Input Jadwal = ";cin>>X[n].jadwal_as;
	cout<<"Input Gaji = ";cin>>X[0].gaji;
	cout<<"Input berhasil\n";
	}
...
void print_asesor(zidan &X){
	...
	cout<<"--------------------------------\n";
	cout<<"No Kode Nama Keahlian Stok\n";
	cout<<"--------------------------------\n";
	for(i=1;i<=n;i++)
		cout<<setw(2)<<i<<setw(6)<<X[i].na_as<<setw(20)<<X[i].nama_as<<setw(8)
		<<X[i].keahlian_as<<endl;
	cout<<"--------------------------------\n";
	}

void add_shchedule(zidan &X){
	string edit; int pos, tambah_stok; bool ada = false; char ya;
	print_asesor(X);
	cout<<"Mengedit data \n";
	...
	if(ada){
		...
		if ((ya == 'y') || (ya == 'Y')){
			cout<<"Masukkan Jadwal Asesment ";cin>>tambah_stok;
			X[pos].jadwal_as=X[pos].jadwal_as+tambah_stok;
...
int main () {
	do{
		system("cls");
		cout<<"Lembaha Sertifikasi Keahlian"<<endl;
		cout<<"1. Add Asesor\n";
		cout<<"2. Add Schedule\n";	
		cout<<"3. Print Asesor\n";
		cout<<"4. Edit Asesor\n";
